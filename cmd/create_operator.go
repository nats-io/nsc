...
func (p *CreateOperatorParams) Validate() error {
	dir, err := os.Getwd()
	if err != nil {
		return err
	}
	...
	if p.kp == nil {
		if KeyPathFlag == "" {
			...
			if !nkeys.IsValidPublicOperatorKey(pk) {
				return fmt.Errorf("private key is not an operator private key")
			}
		}
	}
...
func (p *CreateOperatorParams) Run() error {
	if KeyPathFlag == "" {
...
func createOperatorCmd() *cobra.Command {
	var p CreateOperatorParams
	cmd := &cobra.Command{
		Use:     "operator",
		Short:   "Create an operator configuration directory",
		Example: "create operator --name myoperator <dirpath>",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cmd.Printf("Success! - created operator directory %q\n", p.dir)

			return nil
		},
	}

	cmd.Flags().StringVarP(&p.name, "name", "n", "", "name for the operator, if not specified uses <dirname>")

	return cmd
}

func init() {
	createCmd.AddCommand(createOperatorCmd())
}
